...
exports.init = exports.SRVIterator = exports.resolveSRV = exports.resolve = exports.RR_TYPES = void 0;
 ...
 */
async function resolve(name, type, cd) {
    const args = { name };
    if (type !== undefined) {
        const t = exports.RR_TYPES[type];
        if (t === undefined) {
    ...
    if (cd !== undefined)
        args.cd = cd;
    const uri = (0, urijs_1.default)(apiUrl).query(args);
...
exports.SRVIterator = SRVIterator;
function init(url, logger) {
    apiUrl = url;
    if (logger)
        debug = logger;
    else
