...
function abort(text: string, response?: any) {
 ...
 */
export async function resolve(name: string, type?: string, cd?: boolean) {
    const args: any = { name };
    ...
    if (d.Answer === Response.NXDomain) return [];
    return d.Answer || [];
}
 ...
 */
function parseSRV(record: string) {
    const data = record.split(' ').filter(el => el.length > 0);
    if (data.length != 4) throw new Error(`Malformed SRV record: ${record}`);
 ...
 */
export async function resolveSRV(srv: string) {
    ...
    let d = r.filter((e: any) => e.type == type).map((e: any) => {
        e.data = parseSRV(e.data);
        return e;
    });

    if (d.length == 1 && d[0].data.target === '.') d = [];
    return d;
}
...
export class SRVIterator {
    ...
    * [Symbol.iterator]() {
        ...
        for(const [k, v] of this.recs) {
            ...
            // the beginning.
            const sorted = v.rrs.sort((a: any, b: any) => a.data.weight - b.data.weight);
            ...
            // running sum (rsum) to 0.
                const sum = sorted.reduce((prev: number, cur: any) => prev + (cur.data.weight as number), 0);
...
export function init(url: string, logger?: (...a: any[]) => any) {
    apiUrl = url;
