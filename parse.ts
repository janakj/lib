export type PercentBreakdown<V> = PercentValue<V>[];
export type PercentValue<V> = [ number | string | null, V];
...
export function parsePercentBreakdown<T>(input: T | PercentBreakdown<T>): [number, T][] {
    if (!Array.isArray(input)) {
        return [[100, input]];
    }

    let nulls = 0, sum = 0;
    const data = input.map<[number | null, T]>(([v, ...r]) => {
        if (v === null) {
            nulls++;
            return [v, ...r];
        } else {
    ...
    if (sum !== 100 && nulls === 0)
        if (!nulls) throw new Error(`Percentage values must add up to 100%`);

    if (nulls) {
        data.forEach(v => {
            if (v[0] === null) v[0] = (100 - sum) / nulls;
        });
    }

    return data as [number, T][];
}
